...
const App: React.FC = () => {
  ...
  const handleExportAllData = () => {
    try {
      ...
      link.href = url;
      link.download = 'pingstudio-full-backup.json';
      ...
      console.error('Error exporting full app data', err);
    }
  };
  ...
  const handleImportAllData = (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    reader.onload = e => {
      try {
        ...
        if (parsed.sessions) {
          const cleanedSessions: Session[] = parsed.sessions.map((s: any) => ({
        ...
        if (parsed.journalEntries) {
          setJournalEntries(parsed.journalEntries as JournalEntry[]);
        }

        if (parsed.gear) {
          setGear(parsed.gear as GearItem[]);
        }

        if (parsed.profile) {
          setProfile(parsed.profile as PhotographerProfile);
        }

        if (parsed.bulletinState) {
          setBulletinState(parsed.bulletinState);
        }

        if (parsed.feedbackLog) {
          setFeedbackLog(parsed.feedbackLog as FeedbackEntry[]);
        }
      } catch (err) {
        console.error('Error importing full app data', err);
        alert('Failed to import data. Please ensure the file is a valid PingStudio backup.');
      } finally {
        event.target.value = '';
      }
    };
          ...
          <div className="flex flex-col gap-4 mb-8">
            <div className="flex gap-4">
              <button
                ...
                className="bg-brand-rose text-white text-[10px] font-bold uppercase tracking-[0.2em] px-6 py-3 rounded-sm hover:bg-brand-black transition-all flex items-center gap-2 shadow-sm border border-white/10"
              >
                <i className="fa-solid fa-box-archive"></i> Export ALL Data (JSON)
              </button>

              <label className="cursor-pointer bg-brand-white border border-brand-rose/40 text-brand-rose text-[10px] font-bold uppercase tracking-[0.2em] px-6 py-3 rounded-sm hover:bg-brand-rose hover:text-white transition-all flex items-center gap-2 shadow-sm">
                <i className="fa-solid fa-upload"></i> Import ALL Data (JSON)
                <input
                  type="file"
